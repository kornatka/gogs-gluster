...
glusternode=$(sudo docker inspect ken| jq '.[0].NetworkSettings.IPAddress' | awk -F '"' '{print $2}')

sed -r /g`hostname`/'s/[0-9\.]+'/$glusternode/ /vagrant/template |sudo tee -a /etc/hosts
