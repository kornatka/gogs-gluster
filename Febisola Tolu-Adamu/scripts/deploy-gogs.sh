...
glusternode=$(sudo docker inspect febi| jq '.[0].NetworkSettings.IPAddress' | awk -F '"' '{print $2}')

sed -r /gn`hostname`/'s/[0-9\.]+'/$glusternode/ /vagrant/gluster-hosts-template |sudo tee -a /etc/hosts
